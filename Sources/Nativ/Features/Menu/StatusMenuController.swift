    ...
    init(
        ...
        NSLayoutConstraint.activate([
            ...
            labels.trailingAnchor.constraint(
                lessThanOrEqualTo: selectedImage.leadingAnchor, constant: -6),
...
final class StatusMenuController: NSObject, NSMenuDelegate {
    ...
    private var isObservingLocalModels = false
    ...
    func stop() {
        ...
        if isObservingLocalModels {
            ...
            isObservingLocalModels = false
        }
    }

    func modelStateDidChange() {
        if model.menuIsOpen {
            refreshVisibleMenuState()
        } else {
            rebuildMenu()
        }
    }
        ...
        if model.sessionStatsDisplayMetrics != nil || model.isRunning || sessionStatsAreLoading {
            ...
            let statusMenuItem = NSMenuItem(
                title: model.isRunning
                    ? model.unavailableMetricsText : "Nativ Server is Not Running",
                action: nil,
                keyEquivalent: ""
            )
    ...
    private func modelMenuTooltip(_ localModel: LocalModel) -> String {
        ...
        if let contextSize = localModel.contextSize {
            let formatted = NumberFormatter.localizedString(
                from: NSNumber(value: contextSize), number: .decimal)
    ...
    private func makeServingStatsSubmenu() -> NSMenu {
        ...
        if let latest = metrics.latest {
            submenu.addItem(.separator())
            addSection(
                "Latest Request", entries: NativStats.latestRequestEntries(latest), to: submenu)
        }
    ...
    private func addSection(_ title: String, entries: [StatsEntry], to menu: NSMenu) {
        ...
        for entry in entries {
            menu.addItem(
                makeAlignedStatsItem(label: entry.label, value: entry.value, tooltip: entry.tooltip)
            )
        }
    }
    ...
    private func statsRowView(label: String, value: String, tooltip: String?) -> NSView {
        ...
        NSLayoutConstraint.activate([
            labelField.leadingAnchor.constraint(
                equalTo: row.leadingAnchor, constant: StatsMenuLayout.horizontalPadding),
            labelField.centerYAnchor.constraint(equalTo: row.centerYAnchor),
            labelField.trailingAnchor.constraint(
                lessThanOrEqualTo: valueField.leadingAnchor,
                constant: -StatsMenuLayout.minimumColumnGap),

            valueField.trailingAnchor.constraint(
                equalTo: row.trailingAnchor, constant: -StatsMenuLayout.horizontalPadding),
    ...
    private func menuLabel(
        _ text: String, alignment: NSTextAlignment, lineBreakMode: NSLineBreakMode
    ) -> NSTextField {
...
@MainActor
private final class SessionStatsHostingView<Content: View>: NSHostingView<Content>,
    SessionStatsHighlighting
{
...
private struct SessionActivityPlot: View {
    ...
    private var accessibilityValue: String {
        ...
        return
            "\(promptTokens) prompt and \(generatedTokens) generated tokens over the last 10 minutes"
    }
}
